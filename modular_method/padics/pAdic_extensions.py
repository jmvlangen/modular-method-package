...
def ramified_quadratic_extensions(pAdics=None, ring=None, prime=None,
    ...
    result = [Integer(0) , Integer(0) , Integer(0) , Integer(0) ]
    result[Integer(0) ] = [K(Integer(1) ), pAdics.uniformizer()]
    # Representatives of <pi> / <pi^2>
    # which is all there is if char = 2 and
    # we are not interested in inert extensions
    if unramified and pAdics.characteristic() != Integer(2) :
        result[Integer(2) ] = [K(Integer(1) )]
    if pAdics.characteristic() == Integer(2)  or inert or trivial:
        ...
        _, T._root = find_pAdic_roots(x*y, pAdics=pAdics,
                                      value_tree=T._root, precision=Integer(1) )
        k = pAdics.valuation(Integer(2) )
        if k > Integer(0) :
            Tunr, Tram = find_pAdic_roots(y - x**Integer(2) , pAdics=pAdics,
                                          value_tree=T.root(), precision=Integer(2) *k)
            Tunr = pAdicTree([x,y], root=Tunr)
            Tall = T.remove_variable('x')
            lsall = [N.representative()[Integer(0) ] for N in
                     Tall._root.children_at_level(Integer(2) *k)]
            Tunr_ = Tunr.remove_variable('x')
            lsunr = [N.representative()[Integer(0) ] for N in
                     Tunr_._root.children_at_level(Integer(2) *k)]
            if unramified:
                result[Integer(2) ] = lsunr
            lsquot = _quot_comp(lsall, lsunr, P**(Integer(2) *k))
            lsrepr = [lsclass[Integer(0) ] for lsclass in lsquot]
            result[Integer(0) ] = [K(a1*a2) for a1 in result[Integer(0) ] for a2 in lsrepr]
        else:
            Tunr = T
        if inert or trivial:
            Ttriv, Tiner = find_pAdic_roots(y - x**Integer(2) ,
                                            pAdics=pAdics,
                                            value_tree=Tunr.root(),
                                            precision=Integer(2) *k+Integer(1) )
            Ttriv = pAdicTree([x, y], root=Ttriv)
            Ttriv_ = Ttriv.remove_variable('x')
            lstriv = [N.representative()[Integer(0) ] for N in
                      Ttriv_._root.children_at_level(Integer(2) *k+Integer(1) )]
        if trivial:
            result[Integer(3) ] = lstriv
        if inert:
            Tunr_ = Tunr.remove_variable('x')
            lsunr = [N.representative()[Integer(0) ] for N in
                     Tunr_._root.children_at_level(Integer(2) *k+Integer(1) )]
            lsquot = _quot_comp(lsunr, lstriv, P**(Integer(2) *k+Integer(1) ))
            result[Integer(1) ] = [lsclass[Integer(0) ] for lsclass in lsquot]
    prresult = (True, inert, unramified, trivial)
    result = tuple(result[i] for i in range(Integer(4) ) if prresult[i])
