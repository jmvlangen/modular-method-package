...
class pAdicBase(SageObject):
    ...
    def uniformizer(self):
        ...
        """
        if self._R == ZZ:
            return self._P.gens()[Integer(0) ]
        else:
            for g in self._P.gens():
                if g.ord(self._P) == Integer(1) :
                    return g
    ...
    def _power_series(self, x, prec, F, pi):
        if prec > Integer(0) :
            c0 = F.lift(F(x))
            return [c0] + self._power_series((x-c0)/pi, prec-Integer(1) , F, pi)
        else:
            return []
            
    def power_series(self, x, precision=Integer(10) ):
        ...
        """
        if self.valuation(x) < Integer(0) :
            raise ValueError(str(x) + " is not integral with respect to " +
                             str(self.prime_ideal()))
        if precision not in ZZ or precision <= Integer(0) :
    ...
    def representatives(self, width=Integer(1) ):
        ...
        """
        if width not in ZZ or width <= Integer(0) :
    ...
    def quotient_ring(self, n, names='param'):
        ...
        if self._R == ZZ:
            return ZZ.quotient((self._P**n).gens()[Integer(0) ])
        else:
    ...
    def extension_multiplicity(self, other):
        ...
        if self.is_extension_of(other):
            iota = other.number_field().embeddings(self.number_field())[Integer(0) ]
            return self.valuation(iota(other.uniformizer()))
        elif other.is_extension_of(self):
            iota = self.number_field().embeddings(other.number_field())[Integer(0) ]
    ...
    def _extension_vector_space(self, other):
        ...
        if key not in self._ext:
            ...
            phi = F.free_module(base=G, map=True)[Integer(2) ]
            self._ext[key] = phi
