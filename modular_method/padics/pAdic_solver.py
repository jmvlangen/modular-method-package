...
def find_pAdic_roots(polynomial, pAdics=None, ring=None, prime=None,
                    variables=None, value_tree=None, precision=Integer(20) ,
                    precision_cap=Integer(20) , give_list=False, verbose=False):
...
def _pAdic_hensel_check(f, fder, T, level, step_size, pAdics, verbose=False):
    ...
    if verbose > Integer(1) :
        print("Start loop of %d cycles"%T.count_children_at_level(level-Integer(1) ))
    for node in T.children_at_level(level - Integer(1) ):
        fa = phi(f(node.representative())/(pi**(level-Integer(1) )))
...
def _find_pAdic_roots(f, fder, T, prec, var_prec, m, pAdics, verbose=False,
    ...
    if prec > Integer(0) :
        if give_list: # In this case we want the precision on the root
                      ...
                      # precision
            for myprec in range(Integer(1) , prec+Integer(1) ):
                if T.is_empty():
                    if quit_on_empty:
                        return Tno
                    level_result = (T, T)
                    ...
                    level = ceil(myprec / m)
                    step_size = myprec - m *(level - Integer(1) )
                    level_result = _pAdic_level_check(f, fder, T,
                                                      level,
                                                      step_size,
                                                      pAdics,
                                                      verbose=verbose)
                T = level_result[Integer(0) ]
                if len(level_result) > Integer(1) :
                    Tno.append(level_result[Integer(1) ])
        else: # In this case we can maximize step_size so we can loop
              # over the precision of the variables
            for level in range(Integer(1) ,var_prec+Integer(1) ):
                if T.is_empty():
                    return T, Tno
                else:
                    step_size = min(m, prec-m*(level-Integer(1) ))
                    level_result = _pAdic_level_check(f, fder, T,
                                                      level,
                                                      step_size,
                                                      pAdics,
                ...
                if len(level_result) > Integer(1) :
                    if give_list:
                        ...
                        Tno.merge(level_result[Integer(1) ])
    if give_list:
        Tno.append(T)
        return Tno
    else:
...
def _init_precision(precision, precision_cap, least_power, multiplicity,
    ...
    if precision > multiplicity * precision_cap and verbose >= Integer(0) :
