...
def field_with_root(K, a, names='sqrt_a', give_embedding=False):
    ...
    if a.is_square():
        ...
        L = K.extension(x**Integer(2)  - a, names=names).absolute_field(names=names)
        if give_embedding:
...
def write_as_extension(phi, give_map=False, names=None):
    ...
    if not L.is_absolute():
        ...
        phi = _concat_maps(phi, L.structure()[Integer(1) ])
        concat_map = True
    else:
        concat_map = False
    if L == QQ:
    ...
    for g in f.change_ring(K).factor():
        if g[Integer(0) ].change_ring(phi)(L.gen()) == Integer(0) :
            if names is None:
                names = L.variable_name()
            if K.variable_name() == names:
                for n in range(len(names)):
                    if names[n:].isdigit():
                        names = names[:n] + str(Integer(names[n:]) + Integer(1) )
                        break
                else:
                    names = names + "1"
            M = K.extension(g[Integer(0) ], names=names)
            if give_map:
                phi = L.hom([M.gen()])
                if concat_map:
                    phi = _concat_maps(L.structure()[Integer(1) ], phi)
                return M, phi
            else:
                return M
...
def composite_field(K1, K2, give_maps=False, names=None):
    ...
    if give_maps:
        to_K1 = _concat_maps(to_K1, K1[Integer(1) ])
        K1 = K1[Integer(0) ]
        to_K2 = _concat_maps(to_K2, K2[Integer(1) ])
        K2 = K2[Integer(0) ]
...
def _composite_field(K0, K1, K2, K1or, K2or, to_K1, to_K2, give_maps,
    ...
    g = f.change_ring(K1).factor()[Integer(0) ][Integer(0) ]
    if g.degree() * K1.absolute_degree() == K2.absolute_degree():
        if give_maps:
            if K0 == QQ:
                ...
                i1 = [i for i in K0.embeddings(K1or) if to_K1(i(a0)) == a0][Integer(0) ]
                i2 = [i for i in K0.embeddings(K2or) if to_K2(i(a0)) == a0][Integer(0) ]
            iota = [i for i in K1or.embeddings(K2or) if i(i1(a0)) == i2(a0)][Integer(0) ]
            if K2or.is_absolute():
                return K2or, iota, K2or.hom(K2or)
            else:
                K = K2or.absolute_field(names=K2or.variable_name())
                iota2 = _write_as_im_gen_map(K.structure()[Integer(1) ])
    ...
    if give_maps:
        ...
        if not K.is_absolute():
            ...
            K1_to_K = _concat_maps(K1_to_K, K.structure()[Integer(1) ])
            K2_to_K = _concat_maps(K2_to_K, K.structure()[Integer(1) ])
        return (K,
                _concat_maps(to_K1, K1_to_K),
                _concat_maps(to_K2, K2_to_K))
    else:
...
def common_embedding_field(K1, K2, give_maps=False, names=None):
    ...
    K1_to_gal = _write_as_im_gen_map(K1.embeddings(K1gal)[Integer(0) ])
    if K01 != QQ:
        ...
        K1_to_gal = [phi for phi in K1.embeddings(K1gal) if
                     all(from_K1gal(phi(a)) == K1_to_gal(a) for a in K1.gens())][Integer(0) ]
...
def intersection_field(K1, K2, L=None, give_maps=False, names=None):
    ...
    if n == Integer(1) :
        if give_maps:
            ...
            return QQ
    b1 = vector([B1 * b[Integer(0) :n1] for b in B.basis()])
    t = Integer(0) 
    while True:
        t += Integer(1) 
        for ls in Partitions(t+n, length=n):
            for v in Permutations(ls):
                v = vector(list(v)) - vector(len(v)*[Integer(1) ])
                alpha1 = b1 * v
                ...
                if f.degree() == n:
                    ...
                    from_opt = from_opt * Kopt.structure()[Integer(0) ]
                    to_opt = Kopt.structure()[Integer(1) ] * to_opt
                    if give_maps:
