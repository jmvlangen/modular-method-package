...
class FreyCurve(EllipticCurve_generic):
    ...
    def __init__(self, curve, parameter_ring=ZZ, condition=None):
        ...
        if self._R == ZZ:
            self._R_to_base = QQ.embeddings(base)[Integer(0) ]
        else:
            self._R_to_base = self._R.number_field().embeddings(base)[Integer(0) ]
    ...
    def primes_of_possible_additive_reduction(self):
        ...
        if K.is_subring(QQ):
            ...
            elif n == Integer(2)  and c4.is_homogeneous() and D.is_homogeneous():
                print("Warning: Assuming that " + str(self.parameters()[Integer(0) ]) +
                       " and " + str(self.parameters()[Integer(1) ]) + " are coprime.")
    ...
    def _initial_tree(self, prime, condition=None, verbose=False, precision_cap=Integer(20) ):
    ...
    def local_data(self, prime, condition=None, verbose=False,
        ...
        Tp = self._initial_tree(pAdics.prime_below(self._R),
                                condition=condition,
                                verbose=(verbose-Integer(1)  if verbose>Integer(0)  else verbose),
                                precision_cap=precision_cap)
        ...
        if len(result) == Integer(1) :
            return result[Integer(0) ][Integer(0) ]
        else:
            return result
    ...
    def minimal_model(self, prime, isomorphism=False, condition=None,
                      verbose=False, precision_cap=Integer(20) ):
        ...
        if isomorphism:
            ...
            return apply_to_conditional_value(lambda x: x[Integer(0) ],
                                              result)
    ...
    def reduction(self, prime, model=None, condition=None,
        ...
        T = C.pAdic_tree(pAdics=pAdicsR,
                         verbose=(verbose if verbose <= Integer(0)  else verbose - Integer(1) ),
        ...
        val = max(Integer(1) , min(Integer(1)  - E_val, T_val, precision_cap))
    ...
    def kodaira_symbol(self, prime, condition=None, verbose=False,
        ...
        if len(result) == Integer(1) :
            return result[Integer(0) ][Integer(0) ][Integer(0) ]
        else:
            return ConditionalValue([(val[Integer(0) ], con) for val, con in result])
    ...
    def has_additive_reduction(self, prime, condition=None, verbose=False,
        ...
        red_type = self.reduction_type(prime, verbose=verbose,
                                       condition=condition,
                                       precision_cap=precision_cap)
        return apply_to_conditional_value(lambda x: (x == Integer(0) ), red_type)
    ...
    def has_split_multiplicative_reduction(self, prime, condition=None,
        ...
        red_type = self.reduction_type(prime, verbose=verbose,
                                       condition=condition,
                                       precision_cap=precision_cap)
        return apply_to_conditional_value(lambda x: (x == Integer(1) ), red_type)
    ...
    def has_multiplicative_reduction(self, prime, condition=None,
        ...
        red_type = self.reduction_type(prime, verbose=verbose,
                                       condition=condition,
                                       precision_cap=precision_cap)
        return apply_to_conditional_value(lambda x: (x == Integer(1)  or x == -Integer(1) ),
                                          red_type)
    ...
    def _trace_of_frobenius(self, pAdics, red_type, condition, verbose,
        ...
        if red_type is None:
            result = {}
            computed = {}
            for N in T.children_at_level(Integer(1) ):
                ...
                result[ap].merge(N)
            return result
        elif red_type == Integer(1)  or red_type == -Integer(1) :
            return {red_type*(len(pAdics.residue_field()) + Integer(1) ): T}
        else:
            raise ValueError("Can not compute trace of frobenius " +
                             "if the curve has additive reduction.")

    def trace_of_frobenius(self, prime, power=Integer(1) , condition=None,
                           verbose=False, precision_cap=Integer(20) ):
    ...
    def newform_candidates(self, bad_primes=None, condition=None,
                           algorithm='sage', verbose=False, precision_cap=Integer(20) ,
                           path=None):
        ...
        """
        if bad_primes is None and verbose >= Integer(0) :
    ...
    def newforms(self, condition=None, bad_primes=None, algorithm='sage',
                 primes=Integer(50) , verbose=False, precision_cap_conductor=Integer(20) ,
                 precision_cap_reduction=Integer(1) , path=None):
        ...
        if primes in ZZ and primes > Integer(0) :
...
class FreyQcurve(FreyCurve, Qcurve):
    ...
    def _add_isogenies_of_degree(self, degree, verbose=False):
        ...
        for g, e in fd.factor():
            if g.degree() != Integer(1) :
                continue
            g = g.change_ring(F) / F(g[Integer(1) ])
            psi = E.isogeny(g)
            Ed = psi.codomain()
            jd = Ed.j_invariant()
            for sigma in G:
                ...
                if jd == Integer(0) :
                    m, um = Integer(6) , c6d/c6s
                elif jd == Integer(1728) :
                    m, um = Integer(4) , c4d/c4s
                else:
                    m, um = Integer(2) , (c6d*c4s)/(c6s*c4d)
                if um.numerator().monomials() != um.denominator().monomials():
                    # Can not be an element of K
                    continue
                m0 = um.numerator().monomials()[Integer(0) ]
                ...
                for gu, e in fu.factor():
                    L = K.extension(gu, names=('lu',)); (lu,) = L._first_ngens(1)
                    RL = R.change_ring(L)
                    ...
                    s = (lu*EsL.a1() - EdL.a1())/Integer(2) 
                    r = (lu**Integer(2) *EsL.a2() - EdL.a2() + s**Integer(2)  + s*EdL.a1())/Integer(3) 
                    t = (lu**Integer(3) *EsL.a3() - EdL.a3() + r*EdL.a1())
                    if (lu**Integer(4) *EsL.a4() != (EdL.a4() - s*EdL.a3() + Integer(2) *r*EdL.a2()
                                           - (t + r*s)*EdL.a1() + Integer(3) *r**Integer(2)  - Integer(2) *s*t) or
                        lu**Integer(6) *EsL.a6() != (EdL.a6() + r*EdL.a4() + r**Integer(2) *EdL.a2() +
                                           r**Integer(3)  - t*EdL.a3() - t**Integer(2)  - r*t*EdL.a1())):
                        continue
    ...
    def isogeny_x_map(self, sigma, change_ring=None):
        ...
        result = (Fx(sum(sum(iota(cf).list()[Integer(0) ] * R(m)
                             for cf, m in num.monomial_coefficient(S.monomial(*e)))
                         * x**e[Integer(0) ] for e in num.exponents())) /
                  Fx(sum(sum(iota(cf).list()[Integer(0) ] * R(m)
                             for cf, m in den.monomial_coefficient(S.monomial(*e)))
                         * x**e[Integer(0) ] for e in den.exponents())))
    ...
    def _minimal_a_invariants(self, from_min):
        ...
        return [sum(iota(a.monomial_coefficient(m)).list()[Integer(0) ] * R(m)
    ...
    def _minimal_isogeny_x_maps(self, Kmin, from_min, min_map):
        ...
        for s in G:
            ...
            Fsnum = sum(sum(iota(cf.monomial_coefficient(mR)).list()[Integer(0) ] *
                            R(mR) for mR in cf.monomials()) *
                            S(mS) for cf, mS in
                        [(Fsnum.monomial_coefficient(mS), mS)
                         for mS in Fsnum.monomials()])
            Fsden = sum(sum(iota(cf.monomial_coefficient(mR)).list()[Integer(0) ] *
                            R(mR) for mR in cf.monomials()) *
                            S(mS) for cf, mS in
    ...
    def conductor_exponent(self, prime, condition=None, verbose=False,
        ...
        if K == self.complete_definition_field():
            ...
            result = tates_algorithm(self, initial_values=Tp,
                                     ...
                                     only_calculate=['conductor'])
        if(len(result) == Integer(1) ):
            result = result[Integer(0) ][Integer(0) ][Integer(0) ]
        else:
            result = ConditionalValue([(val[Integer(0) ], con) for val,con in result])
        if K == self.complete_definition_field():
            for P in Pls:
                if P != prime:
        ...
        if additive_primes is None:
            ...
            for p in K.discriminant().prime_factors():
                for P in K.primes_above(p):
                    if P.ramification_index() > Integer(1)  and P not in additive_primes:
        ...
        if isinstance(additive_part, ConditionalExpression):
            ...
            for f in additive_factors:
                for p,e in f.absolute_norm().factor():
                    ...
                    if p in left_factors and left_factors[p] != Integer(0) :
            ...
            for p, e in disc_factors:
                if p in left_factors and left_factors[p] != Integer(0) :
                    ...
                    left_factors[p] = e
            if hasattr(disc_factors, 'unit') and disc_factors.unit() != Integer(1) :
    ...
    def newform_candidates(self, bad_primes=None, condition=None, conjugates=True,
                           algorithm='sage', verbose=False, precision_cap=Integer(20) ,
                           path=None, splitting_map=Integer(0) ):
    ...
    def _newform_candidates(self, levels, condition, conjugates,
        ...
        result = []
        done_levels = {}
        characters = [(eps**(-Integer(1) )).primitive_character()
                      for eps in self.splitting_character('conjugacy')]
        twists = [dirichlet_product(self.twist_character(splitting_map)**(-Integer(1) ),
    ...
    def trace_of_frobenius(self, prime, power=Integer(1) , splitting_map=Integer(0) ,
                           condition=None, verbose=False, precision_cap=Integer(20) ):
            ...
            def compute_trace2(red_type, con2):
                if red_type == None: # Good reduction
                    return self._trace_of_frob_good(P, beta,
                                                    ...
                                                    verbose,
                                                    precision_cap)
                elif red_type == Integer(1)  or red_type == -Integer(1) : # Multiplicative reduction
    ...
    def _trace_of_frob_good(self, P, beta, condition, verbose,
        ...
        def compute_trace1(min_data, con1):
            Emin = min_data[Integer(0) ]
            phi_min = _rational_maps_of_urst(*min_data[Integer(1) ][Integer(1) ])
            psi1 = psi[Integer(0) ](phi_min), psi[Integer(1) ](phi_min)
            def compute_trace2(smin_data, con2):
                sEmin = smin_data[Integer(0) ]
                sphi_min = _rational_maps_of_urst(*smin_data[Integer(1) ][Integer(0) ])
                phi = sphi_min[Integer(0) ](psi1), sphi_min[Integer(1) ](psi1)
                ...
                sE_val = ZZ(min(pAdicsK.valuation(cf)
                                ...
                                for cf in a.coefficients())) // ext
                T_val = T.minimum_full_level()
                val = max(Integer(1) , min(Integer(1)  - E_val, Integer(1)  - sE_val, T_val,
                                 precision_cap))
                for node in T.children_at_level(val):
                    ...
                    # separable
                    phi0num = phi[Integer(0) ].numerator().change_ring(fill)
                    phi0den = phi[Integer(0) ].denominator().change_ring(fill)
                    ...
                    if F.derivative(x) == Integer(0) :
                        ...
                        phi1num = phi[Integer(1) ].numerator().change_ring(fill)
                        phi1den = phi[Integer(1) ].denominator().change_ring(fill)
                        ...
                        if p == Integer(2) :
                            ...
                            R3 = (Integer(4) *x**Integer(3)  + Ered.b2()*x**Integer(2)  + Integer(2) *Ered.b4()*x +
                                  Ered.b6())
                            l = _scalar_of_rational_maps(phi0, phi1, Ered,
                                                         sEred)
                            c2 = (l * R3**((p + Integer(1) )/Integer(2) ) -
                        ...
                        result = beta(Frob)**(-Integer(1) ) * apE
