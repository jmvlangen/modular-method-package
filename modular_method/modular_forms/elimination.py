        ...
        for curve in curves:
            ...
            if parameters is None:
                ...
                ring = curve._R
                  ...
                  ring != curve._R):
                raise ValueError(str(curves[Integer(0) ]) + " and " + str(curve) +
                                 " don't have the same parameters")
    newforms = _init_newform_list(newforms, curves)
    if condition is None:
        condition = curves[Integer(0) ]._condition
...
def _init_traces(curves, condition, prime, primes, powers,
    ...
    """
    K = curves[Integer(0) ]._R.fraction_field()
...
def _init_newform_list(newforms, curves):
    ...
    """
    if newforms is None and len(curves) == Integer(1) :
        newforms = apply_to_conditional_value(lambda x: list(x),
                                              curves[Integer(0) ].newform_candidates())
    ...
    for i in range(len(newforms)):
        if not isinstance(newforms[i], tuple):
            if len(curves) != Integer(1) :
                raise ValueError("Expected " + str(len(curves)) +
                                 " newforms per entry, but got 1")
            newforms[i] = (newforms[i], ZZ(Integer(0) ))
        if len(newforms[i]) == len(curves) + Integer(1) :
            pass
        elif len(newforms[i]) == len(curves):
            newforms[i] = tuple(list(newforms[i]) + [Integer(0) ])
        else:
            raise ValueError("Expected " + str(len(curves)) +
...
def eliminate_by_trace(curves, newforms, prime, B=Integer(0) , condition=None,
                       use_minpoly=False, precision_cap=Integer(1) ,
                       verbose=False):
...
def _single_elimination(E, KE, LE, nfs, p, prime, pE, B, Bprod, C,
    ...
    nE = len(E)
    Bold = nfs[-Integer(1) ]
    if (gcd(Bprod, Bold) == Integer(1)  or (B != Integer(0)  and Bold == Integer(0) )):
        # Nothing to do
        return Bold
    Kf = tuple(nfs[i].base_field() for i in range(nE))
    Kcom = tuple(common_embedding_field(KE[i], Kf[i], give_maps=True)
                 for i in range(nE))
    KphiE = tuple(Kcom[i][Integer(1) ] for i in range(nE))
    Kphif = tuple(Kcom[i][Integer(2) ] for i in range(nE))
    Kcom = tuple(Kcom[i][Integer(0) ] for i in range(nE))
    ...
    apE_ls = _init_traces(E, C, prime, pE, powE, prec_cap,
                          (verbose - Integer(1)  if verbose > Integer(0)  else verbose))
    ...
    if use_minpoly:
        ...
        Lcom = tuple(common_embedding_field(LE[i], Lf[i], give_maps=True)
                     for i in range(nE))
        LphiE = tuple(Lcom[i][Integer(1) ] for i in range(nE))
        Lcom = tuple(Lcom[i][Integer(0) ] for i in range(nE))
...
def _eliminate_by_trace(curves, newforms, prime, B, C, use_minpoly,
    ...
    for nfs in newforms:
        Bnew = _single_elimination(curves, KE, LE, nfs, p, prime, pE,
                                   ...
                                   verbose)
        if abs(Bnew) != Integer(1) :
            result.append(tuple([nfs[i] for i in range(nE)] + [Bnew]))
    _init_traces.clear_cache()
    return result

def eliminate_by_traces(curves, newforms, condition=None, primes=Integer(50) ,
                        use_minpoly=False, precision_cap=Integer(1) ,
                        verbose=False):
    ...
    if not (isinstance(primes, list) and
            all((K == QQ and p in ZZ and p > Integer(0)  and p.is_prime()) or
...
def _eliminate_by_traces(curves, newforms, condition, primes,
    ...
    """
    for prime in primes:
        newforms = _eliminate_by_trace(curves, newforms, prime, Integer(0) ,
                                       condition, use_minpoly,
                                       precision_cap, verbose)
    return newforms

def kraus_method(curves, newforms, l, polynomials, primes=Integer(200) ,
                 condition=None, use_minpoly=False, precision_cap=Integer(1) ,
                 verbose=False):
    ...
    curves, newforms, condition = _init_elimination_data(curves, newforms,
                                                         condition)
    if not (l in ZZ and l > Integer(0)  and l.is_prime()):
...
def _kraus_method(curves, newforms, l, polynomials, primes, condition,
    ...
    """
    base_field = curves[Integer(0) ]._R.fraction_field()
    variables = curves[Integer(0) ].base().gens()
...
def kraus_condition(l, polynomials, variables, base_field,
    ...
    if len(kraus_data) == Integer(0)  and error:
...
def _kraus_data_iterator(l, polynomials, base_prime):
    ...
    for i in range(len(polynomials)):
        ...
        for P in _primes_1_mod_l(field, base_prime, l):
            ...
            order_u = F.cardinality() - Integer(1) 
            order_ul = order_u / l
            pows = [F(Integer(0) )] + [F(u**(k*l)) for k in range(order_ul)]
            yield poly, F, pows
...
def _primes_1_mod_l(K, p, l):
    ...
    """
    if K == QQ:
        if mod(p, l) == Integer(1) :
            return [p]
        else:
            return []
    else:
        return [P for P in K.primes_above(p)
                if mod(P.residue_field().cardinality(), l) == Integer(1) ]
...
def _eliminate_primes(newforms, ls):
    ...
    result = []
    for nfs in newforms:
        if nfs[-Integer(1) ] != Integer(0) :
            nfs = list(nfs)
            for l in ls:
                nfs[-Integer(1) ] = ZZ(nfs[-Integer(1) ] / (l**nfs[-Integer(1) ].ord(l)))
            nfs = tuple(nfs)
        if abs(nfs[-Integer(1) ]) != Integer(1)  :
            result.append(nfs)
    return result
...
def _combine_newforms(*newforms):
    ...
    return [nfs for nfs in (sum((item[:-Integer(1) ] for item in items), ()) +
                            (gcd([item[-Integer(1) ] for item in items]),)
