...
class power_analyzer(SageObject):
    ...
    def factor(self, K):
        ...
        """
        return self._f.change_ring(K).factor()[Integer(0) ][Integer(0) ]
    ...
    def relations(self, K, g=None):
        ...
        G = K.galois_group()
        G_ls = [s for s in G if s != G(Integer(1) )]
        result = []
        for s_ls in Combinations(G_ls, n - Integer(1) ):
            candidate = g * product(g.change_ring(s.as_hom()) for s in s_ls)
            test = (candidate / self._f)
            if test.denominator() == Integer(1) :
                # We found a relation
                result.append(([G(Integer(1) )] + list(s_ls),
    ...
    def bad_primes(self, K, g=None):
        ...
        result = reduce(lambda I, J: I + J,
                        [reduce(lambda I, J: I.intersection(J),
                                [K.ideal(coprime[t*s**(-Integer(1) )])
                                 for s, t in Combinations(s_ls, Integer(2) )],
                                K.ideal(C))
                         for s_ls, C in self.relations(K, g)])
        if result == K.ideal(Integer(0) ):
            return Integer(0) 
        else:
    ...
    def constant_coeff(self, K, g, bad_primes_val, l):
        ...
        J = (CJ**(-Integer(1) )).ideal()
        return (I * (J**l)).gens_reduced()[Integer(0) ]
    ...
    def unit_relation(self, K, g, c, relation, l):
        ...
        l_part = l_part.gens_reduced()[Integer(0) ]
        U = K.unit_group()
    ...
    def unit_coeffs(self, K, g, l, c=None, bad_primes_val=None):
        ...
        M, v = [block_matrix(n, Integer(1) , Mi) for Mi in zip(*lin_rel)]
    ...
    def prime_data(self, K, g, l, primes, c=None, bad_primes_val=None):
        ...
        for p in primes:
            ...
            for cf in u_dict:
                ...
                if case in done_cases:
                    ...
                    arg_ls = tuple(tuple(arg) for arg in mrange([p]*n)
                                   if gcd(arg) != Integer(0)  and
                                   all(xgl_arg == Integer(0)  or
                                       l.divides(xgl_arg.log(xp))
                                       for xgl_arg
                                       in (xgl(arg) for xgl in xgl_poly)))
                    if len(arg_ls) == Integer(0) :
                        removed.append(cf)
                    else:
                        u_dict[cf][p] = arg_ls
                    done_cases[case] = cf
            for cf in removed:
                del u_dict[cf]
        if len(u_dict) == Integer(0) :
            return {}
        primes = u_dict[u_dict.keys()[Integer(0) ]].keys()
...
def polynomial_split_on_basis(f, B):
    ...
    monomials = [[cf * R(m)
                  for cf in (M**(-Integer(1) )*vector(f.monomial_coefficient(m).list()))]
