...
class PolynomialCondition(Condition_base):
    ...
    def __init__(self, polynomial, precision=Integer(20) ):
    ...
    def never(self):
        ...
        """
        return (self._f.is_constant() and self._f != Integer(0) )
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False,
        ...
        if K0.is_subring(QQ):
            ...
            iota = K0.hom([a.minpoly().change_ring(K).roots()[Integer(0) ][Integer(0) ] for a in K0.gens()], K)
    ...
    def _latex_(self):
        return latex(self.polynomial()) + " = " + latex(Integer(0) )
...
class ExistsCondition(PolynomialCondition):
    ...
    def __init__(self, polynomial, variables, precision=Integer(20) ):
    ...
    def _repr_(self):
        ...
        if n > Integer(0) :
            result += " for some "
            for i in range(n):
                if i > Integer(0)  and i == n - Integer(1) :
                    result += "and "
...
class CongruenceCondition(PolynomialCondition):
    ...
    def __init__(self, polynomial, modulus):
        ...
        if hasattr(self._mod, 'is_principal') and self._mod.is_principal():
            self._mod = self._mod.gens_reduced()[Integer(0) ]    
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None,
        ...
        result = PolynomialCondition.pAdic_tree(self, pAdic_tree=pAdic_tree,
                                                ...
                                                precision=precision,
                                                **kwds)
        if complement and precision == Integer(0) :
            return result[Integer(0) ], result[Integer(0) ]
        else:
            return result
...
class PowerCondition(PolynomialCondition):
    ...
    def __init__(self, polynomial, least_exp=Integer(1) ):
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False,
        ...
        T1, T0 = PolynomialCondition.pAdic_tree(self, pAdic_tree=pAdic_tree,
                                                ...
                                                verbose=verbose, precision=Integer(1) ,
                                                **kwds)
    ...
    def _latex_(self):
        return (latex(self.polynomial()) + " = " +
                "x_{" + self._x_str()[Integer(1) :] + "}" +
                "^{n_{" + self._n_str()[Integer(1) :] + "}}")
...
class OrderCondition(PolynomialCondition):
    ...
    def __init__(self, polynomial, n=Integer(0) ):
...
class SquarefreeCondition(PolynomialCondition):
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False,
        ...
        TN, TY = PolynomialCondition.pAdic_tree(self, pAdic_tree=pAdic_tree,
                                                pAdics=pAdics, complement=True,
                                                verbose=verbose, precision=Integer(2) ,
                                                **kwds)
        if complement:
...
class CoprimeCondition(Condition_base):
    ...
    def __init__(self, variables, n=Integer(2) ):
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False, **kwds):
        ...
        indices = tuple(tree_vars.index(var)
                        for var in self.variables()
                        if var in tree_vars)
        for node in T.children_at_level(Integer(1) ):
            if (sum(c == Integer(0)  for i,c in enumerate(node.quotient_tuple())
...
class AndCondition(Condition_base):
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False, **kwds):
        ...
        T2 = self._right.pAdic_tree(pAdic_tree=pAdic_tree, pAdics=pAdics,
                                    complement=complement, **kwds)
        if complement:
            return T1[Integer(0) ].intersection(T2[Integer(0) ]), T1[Integer(1) ].union(T2[Integer(1) ])
        else:
    ...
    def _repr_(self):
        ...
        right_str = right_str[Integer(0) ].lower() + right_str[Integer(1) :]
        left_str = self._left._repr_()
        if left_str.endswith('.'):
            left_str = left_str[:-Integer(1) ]
...
class OrCondition(Condition_base):
    ...
    def pAdic_tree(self, pAdic_tree=None, pAdics=None, complement=False, **kwds):
        ...
        T2 = self._right.pAdic_tree(pAdic_tree=pAdic_tree, pAdics=pAdics,
                                    complement=complement, **kwds)
        if complement:
            return T1[Integer(0) ].union(T2[Integer(0) ]), T1[Integer(1) ].intersection(T2[Integer(1) ])
        else:
            return T1.union(T2)
...
class TreeCondition(Condition_base):
    ...
    def _repr_len(self, max_item=Integer(50) , max_char=Integer(1000) ):
        ...
        if hasattr(modulus, 'is_principal') and modulus.is_principal():
            modulus = modulus.gens_reduced()[Integer(0) ]
        if len(values) <= max_item:
            result = (str(self.variables() if l > Integer(1)  else self.variables()[Integer(0) ]) +
                      " == ")
            for i, value in enumerate(values):
                ...
                if i > Integer(0) :
                    result += ", "
                result += str(value if l > Integer(1)  else value[Integer(0) ])
            result += (" mod " +
                       (modulus._repr_short() if hasattr(modulus, '_repr_short')
                        else str(modulus)))
            if len(result) <= max_char:
                return result
        return (str(self.variables() if l > Integer(1)  else self.variables()[Integer(0) ]) +
    ...
    def _repr_short(self):
        return self._repr_len(max_item=Integer(10) , max_char=Integer(40) )
    ...
    def _latex_(self):
        ...
        if hasattr(modulus, 'is_principal') and modulus.is_principal():
            modulus = modulus.gens_reduced()[Integer(0) ]
        result = (latex(self.variables() if l > Integer(1)  else self.variables()[Integer(0) ]) +
                  " = ")
        for i, value in enumerate(values):
            if i > Integer(0) :
                result += ", "
            result += latex(value if l > Integer(1)  else value[Integer(0) ])
...
class ConditionalValue(SageObject):
    ...
    def __init__(self, val_con):
        ...
        """
        self._vals = tuple(vc[Integer(0) ] for vc in val_con)
        self._con = tuple(vc[Integer(1) ] for vc in val_con)
        for c in self._con:
    ...
    def no_value(self):
        ...
        """
        return len(self._vals) == Integer(0) 

    def _repr_lines(self):
        if self.no_value():
            return []
        result = [str(val) for val in self._vals]
        l = max(len(r) for r in result) + Integer(1) 
...
class ConditionalExpression(SageObject):
    SUM_OPERATOR = ('+', '+', Integer(0) )
    MINUS_OPERATOR = ('-', '-', RealNumber('0.5') )
    PRODUCT_OPERATOR = ('*', '\\cdot', Integer(2) )
    DIVISION_OPERATOR = ('/','/', RealNumber('2.5') )
    EXPONENT_OPERATOR = ('^', '^', RealNumber('4.5') )
    ...
    def _factor_side(self, side):
        ...
        if hasattr(side, 'factor'):
            ...
            if hasattr(F, 'unit') and F.unit() != Integer(1) :
                result[F.unit()] = Integer(1) 
            return result
        return {side: Integer(1) }
    ...
    def factors(self):
        ...
        if self._op == ConditionalExpression.PRODUCT_OPERATOR:
            ...
            for f in extra:
                if f in result and result[f] != Integer(0) :
                    result[f] = result[f] + extra[f]
                elif extra[f] != Integer(0) :
                    result[f] = extra[f]
            return result
        if self._op == ConditionalExpression.DIVISION_OPERATOR:
            ...
            for f in extra:
                if f in result and result[f] != Integer(0) :
                    ...
                    result[f] = Integer(0)  - extra[f]
            return result
        if self._op == ConditionalExpression.EXPONENT_OPERATOR:
            result = self._factor_side(self._left)
            for f in result:
                if result[f] == Integer(1) :
                    result[f] = self._right
                elif self._right != Integer(1) :
    ...
    def _repr_side(self, side, vals, bracket_level):
        ...
        if isinstance(side, ConditionalValue):
            vals.append(side)
            return "n" + str(len(vals) - Integer(1) )
    ...
    def _repr_info(self, vals, bracket_level):
        result = ""
        if bracket_level > self._op[Integer(2) ]:
            result += "("
        result += self._repr_side(self._left, vals, floor(self._op[Integer(2) ]))
        result += self._op[Integer(0) ]
        result += self._repr_side(self._right, vals, ceil(self._op[Integer(2) ]))
        if bracket_level > self._op[Integer(2) ]:
            result += ")"
        return result
    ...
    def _repr_(self):
        vals = []
        result = self._repr_info(vals, Integer(0) )
        if len(vals) > Integer(0) :
            result += "\n where \n"
            front_space = ceil(ZZ(len(vals)).log(Integer(10) )) + Integer(5) 
            for i, val in enumerate(vals):
                lines = val._repr_lines()
                if len(lines) == Integer(0) :
                    if i > Integer(0) :
                        result += "\n"
                    result += "n" + str(i) + " ="
    ...
    def _latex_side(self, side, vals, bracket_level):
        ...
        if isinstance(side, ConditionalValue):
            vals.append(side)
            return "n_{" + str(len(vals) - Integer(1) ) + "}"
        return str(side)
    ...
    def _latex_info(self, vals, bracket_level):
        ...
        if bracket_level > self._op[Integer(2) ]:
            result += "\\left("
        result += self._latex_side(self._left, vals, floor(self._op[Integer(2) ]))
        result += self._op[Integer(1) ]
        result += self._latex_side(self._right, vals, ceil(self._op[Integer(2) ]))
        if bracket_level > self._op[Integer(2) ]:
            result += "\\right)"
        return result
    ...
    def _latex_(self):
        vals = []
        result = self._latex_info(vals,Integer(0) )
        if len(vals) > Integer(0) :
            result += "\n\\\\\\text{ where } \\\\\n"
            for i, val in enumerate(vals):
                if i > Integer(0) :
                    result += " \\\n"
                result += "n_{" + str(i+Integer(1) ) + "} = "
                result += " & " + latex(val)
        return result
...
def apply_to_conditional_value(function, value, singleton=False,
    ...
    if not singleton and len(values) == Integer(1) :
        ...
        for fval, fcon in result:
            if not fcon.never():
                try:
                    ...
                    vals.append(fval)
                    cons.append(fcon)
    if not singleton and len(vals) == Integer(1) :
        return vals[Integer(0) ]
    else:
