...
def solve_integer_problem_with_torsion(M, V, MT, VT, N, all=False):
    ...
    m, n = M.dimensions()
    mt, nt = MT.dimensions()
    M0 = block_matrix(Integer(2) ,Integer(2) ,[M, zero_matrix(m, mt), MT.change_ring(ZZ),
                           diagonal_matrix([N]*mt)])
    V0 = vector(list(V) + list(VT.change_ring(ZZ)))
    MV0 = block_matrix(Integer(1) ,Integer(2) ,[M0,V0.column()])
    B = MV0.right_kernel().basis_matrix().transpose()
    g = B[-Integer(1) ][Integer(0) ]
    c = [Integer(1) ]
    for i in range(Integer(1) , len(B[-Integer(1) ])):
        g, s, t = xgcd(g,B[-Integer(1) ][i])
