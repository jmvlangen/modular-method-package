...
def _independent_cocycle_arguments(G):
    ...
    for s in G_without_1:
        ...
        if (s**Integer(2) , s) in result and (s, s**Integer(2) ) in result and (s, s) in result:
            result.remove((s**Integer(2) , s)) 
...
def function_with_coboundary(G, A, c, action=None):
    ...
    alpha0 = {G.gens()[i]: block_matrix(Integer(1) , m+Integer(1) ,
                                        [(identity_matrix(n) if j == i
                                          else zero_matrix(n))
                                         for j in range(m)] +
                                        [vector([Integer(0) ]*n).column()])
              for i in range(m)}
    def c_to_matrix(s,t):
    ...
    # Building a matrix:
    MV = block_matrix(len(relations), Integer(1) , [alpha0[s] + action[s]*alpha0[t]
    ...
    MF = MVF[:,:-Integer(1) ]
    VF = -MVF[:,-Integer(1) ]
    if VF.dimensions()[Integer(0) ] == Integer(0) :
        VF = vector([])
    else:
        VF = vector(VF)
    MT = MVT[:,:-Integer(1) ]
    VT = -MVT[:,-Integer(1) ]
    if VT.dimensions()[Integer(0) ] == Integer(0) :
        VT = vector([])
    else:
        VT = vector(VT)
    ...
    def alpha(s):
        result = identity
        val = alpha0[s] * block_matrix(Integer(2) ,Integer(1) ,[v.column(), identity_matrix(Integer(1) )])
        for k in range(n):
            result = result * gens[k]**val[k][Integer(0) ]
        return result
    return alpha
...
def hilbert90(K, f):
    ...
    m = (K.ideal(d)*J*I**(-Integer(1) )).smallest_integer()    
    n = d / m
    return a / n
