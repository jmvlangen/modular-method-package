def Euler_factor_modular_form(f, p, twists=[Integer(1) ]):
    ...
    for chip in twists:
        for F in K.galois_group():
            factor = (Integer(1)  - F(chip * ap) * T + F(chip**Integer(2)  * epsp * p) * T**Integer(2) )
...
def _Euler_factor_elliptic_curve_local(E, P, R):
    ...
    if E.has_additive_reduction(P):
        ...
        aP = NP + Integer(1)  - E.reduction(P).count_points()
        return R.one() - aP * x**e + NP * x**(Integer(2) *e)
...
def _Euler_factor_elliptic_curve_local_QQ(E, p, R):
    ...
    if E.has_additive_reduction(p):
        ...
        ap = p + Integer(1)  - E.reduction(p).count_points()
        return R.one() - ap * x + p * x**Integer(2) 
